...
class Database:
    """Simple SQLite wrapper that keeps the existing `get_connection()` API
    used across the app but adds some sane defaults (PRAGMAs), a context
    manager, and helper methods for migrations/initialization.

    The module exports a `db` instance for backward compatibility.
    """
    ...
    def get_connection(self):
        """Return a new sqlite3.Connection with recommended pragmas set.

        Callers are expected to close the connection when done.
        """
        conn = sqlite3.connect(self.db_path, timeout=30)
        conn.row_factory = sqlite3.Row
        # Recommended pragmas for SQLite usage in web apps
    ...
    def init_db(self):
        ...
        # We'll create tables similarly to the previous implementation.
        with self.connection() as (conn, cursor):
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    events_atteded INTEGER DEFAULT 0,
    ...
    def insert_sample_data(self):
        with self.connection() as (conn, cursor):
            ...
            # minimal sample clubs and events can be inserted similarly if desired


# Export a module-level db instance for backwards compatibility
db = Database()